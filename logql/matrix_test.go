...
func TestMatrixStepper(t *testing.T) {
	...
	expected := []promql.Vector{
		...
		},
		{
			...
			promql.Sample{
				T: start.Add(step).UnixNano() / int64(time.Millisecond), F: 0,
				Metric: labels.FromStrings("bazz", "buzz"),
			},
		},
		{
			promql.Sample{
				T:      start.Add(2*step).UnixNano() / int64(time.Millisecond),
				F:      2,
				Metric: labels.FromStrings("foo", "bar"),
			},
			promql.Sample{
		...
		{
			promql.Sample{
				T:      start.Add(3*step).UnixNano() / int64(time.Millisecond),
				F:      3,
				Metric: labels.FromStrings("foo", "bar"),
			},
			promql.Sample{
		...
		{
			promql.Sample{
				T:      start.Add(4*step).UnixNano() / int64(time.Millisecond),
				F:      4,
				Metric: labels.FromStrings("foo", "bar"),
			},
			promql.Sample{
		...
		{
			promql.Sample{
				T:      start.Add(5*step).UnixNano() / int64(time.Millisecond),
				F:      5,
				Metric: labels.FromStrings("foo", "bar"),
			},
			promql.Sample{
		...
		{
			promql.Sample{
				T:      start.Add(6*step).UnixNano() / int64(time.Millisecond),
				F:      0,
				Metric: labels.FromStrings("foo", "bar"),
			},
			promql.Sample{
