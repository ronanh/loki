...
func Test_ParserHints(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		t.Run(tt.expr, func(t *testing.T) {
			...
			require.NoError(t, err)
			v, lbsRes, ok := ex.ForStream(lbs).
				Process(0, slices.Clone(tt.line), labels.EmptyLabels(), 0)
