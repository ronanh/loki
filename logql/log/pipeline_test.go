...
func TestNoopPipeline(t *testing.T) {
	...
	l, lbr, ok := NewNoopPipeline().ForStream(lbs).Process(0, []byte(""), labels.EmptyLabels(), 0)
	require.Equal(t, []byte(""), l)
	require.Equal(t, NewLabelsResult(lbs, labels.StableHash(lbs)), lbr)
	require.Equal(t, true, ok)

	ls, lbr, ok := NewNoopPipeline().ForStream(lbs).ProcessString(0, "", labels.EmptyLabels(), 0)
	require.Equal(t, "", ls)
	require.Equal(t, NewLabelsResult(lbs, labels.StableHash(lbs)), lbr)
	require.Equal(t, true, ok)
}
...
func TestPipeline(t *testing.T) {
	...
	l, lbr, ok := p.ForStream(lbs).Process(0, []byte("line"), labels.EmptyLabels(), 0)
	require.Equal(t, []byte("lbs bar"), l)
	require.Equal(t, NewLabelsResult(lbs, labels.StableHash(lbs)), lbr)
	require.Equal(t, true, ok)

	ls, lbr, ok := p.ForStream(lbs).ProcessString(0, "line", labels.EmptyLabels(), 0)
	require.Equal(t, "lbs bar", ls)
	require.Equal(t, NewLabelsResult(lbs, labels.StableHash(lbs)), lbr)
	require.Equal(t, true, ok)

	l, lbr, ok = p.ForStream(labels.EmptyLabels()).
		Process(0, []byte("line"), labels.EmptyLabels(), 0)
	require.Equal(t, []byte(nil), l)
	require.Equal(t, nil, lbr)
	require.Equal(t, false, ok)

	ls, lbr, ok = p.ForStream(labels.EmptyLabels()).
		ProcessString(0, "line", labels.EmptyLabels(), 0)
	require.Equal(t, "", ls)
	require.Equal(t, nil, lbr)
	require.Equal(t, false, ok)
}
...
func Benchmark_Pipeline(b *testing.B) {
	...
	sp := p.ForStream(lbs)
	for n := 0; n < b.N; n++ {
		resLine, resLbs, resOK = sp.Process(0, line, labels.EmptyLabels(), 0)
	}
}
