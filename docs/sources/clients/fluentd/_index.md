...
The code source of the plugin is located in our [public repository](https://github.com/ronanh/loki/tree/master/cmd/fluentd).
...
There is a Docker image `grafana/fluent-plugin-loki:master` which contains [default configuration files](https://github.com/ronanh/loki/tree/master/fluentd/fluent-plugin-loki/docker/conf). By default, fluentd containers use the configurations but you can also specify your `fluentd.conf` with `FLUENTD_CONF` environment variable.
