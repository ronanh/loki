...
func TestParseTailQuery(t *testing.T) {
	...
	tests := []struct {
		...
		{"bad time", &http.Request{URL: mustParseURL(`?query={foo="bar"}&start=t`)}, nil, true},
		{
			...
			&http.Request{
				URL: mustParseURL(
					`?query={foo="bar"}&start=2016-06-10T21:42:24.760738998Z&limit=h`,
				),
			},
			nil,
			true,
		},
		{
			...
			&http.Request{
				URL: mustParseURL(
					`?query={foo="bar"}&time=2016-06-10T21:42:24.760738998Z&limit=100&delay_for=fw`,
				),
			}, nil, true,
		},
		{
			...
			&http.Request{
				URL: mustParseURL(
					`?query={foo="bar"}&time=2016-06-10T21:42:24.760738998Z&limit=100&delay_for=20`,
				),
			}, nil, true,
		},
		{
			...
			&http.Request{
				URL: mustParseURL(
					`?query={foo="bar"}&start=2017-06-10T21:42:24.760738998Z&limit=1000&delay_for=5`,
				),
